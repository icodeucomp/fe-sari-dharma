    ...
    <div className="space-y-4 divide-y-2 divide-primary">
      ...
      <div className="pt-4 space-y-4">
        {paketKesehatan.map((paket, index) => (
          <Motion tag="div" initialY={50} animateY={0} duration={0.5} delay={index * 0.1} key={paket.id} className="pb-4 space-y-2 border-b border-gray/30">
            <Img src={`${process.env.NEXT_PUBLIC_API_URL}/storage/${paket.foto}`} alt={paket.nama_paket} className="w-full min-h-40" cover />
            ...
            <div className="space-y-2 text-dark">
              <span className="text-xs sm:text-sm font-light">*Berlaku s/d {moment(paket.berlaku_sampai).format("DD MMMM YYYY")}</span>
              <h3 className="font-semibold line-clamp-2">{paket.nama_paket}</h3>
              <p className="line-clamp-2 text-gray text-sm max-w-xs text-justify">{paket.deskripsi}</p>
              <Link href={`/media-informasi/paket-promo/${paket.slug}/${paket.id}`} className="block">
